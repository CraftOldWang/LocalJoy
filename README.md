...
面向本地生活场景的 Java + React 学习项目。以 **店铺商品 → 秒杀受理 → 异步建单 → 模拟支付 / 超时关闭** 为主线，展示 Redis 库存预扣、RocketMQ 异步处理、幂等补偿、商品缓存与滑动窗口限流。

主站已改为 **React**，不再围绕原版黑马点评的笔记、签到、消息功能组织页面；原版 Vue 页面保留为存档。支付仅更新本地订单状态，**不会发生真实扣款**。
...
这是学习与面试演示项目，不是可直接公网运营的交易系统。课程遗留的商品、店铺写入与上传等接口没有完整管理员权限体系；没有真实支付验签、退款、核销，也没有订单价格快照。
