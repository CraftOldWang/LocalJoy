...
export function Orders() {
  ...
  return <><section className="intro row spread"><div><p className="eyebrow">YOUR LITTLE JOYS</p><h1>每一份期待，都有记录。</h1><p className="muted">查看抢购结果，体验本地模拟或支付宝沙箱支付。</p></div><button onClick={resource.reload} disabled={resource.loading}>刷新订单</button></section>
...
export function Order() {
  ...
  return <><Link className="back-link" to="/orders">← 返回我的订单</Link><section className="intro"><p className="eyebrow">ORDER PROGRESS</p><h1>{order ? orderStatus(order) : accepted ? '正在确认你的好物' : '订单详情'}</h1><p className="order-id">订单号 {id}</p></section>
    {!order && accepted && validId(id) ? <section className="card state" aria-live="polite" aria-busy={view.checking}>{view.attempts < 30 && <span className="spinner" aria-hidden="true"/>}<h2>{view.attempts < 30 ? '抢购申请已受理' : '暂未查到订单结果'}</h2><p className="muted">{view.attempts < 30 ? '正在查询建单结果，请勿重复提交。' : '自动查询已暂停，可手动刷新，或稍后查看我的订单。'}</p>{view.error && <p className="help">{view.error}</p>}<button onClick={refresh} disabled={view.checking}>刷新结果</button></section> : !order ? <State {...view} onRetry={refresh}/> : <section className="card card-body order-detail">
      ...
      <div className="row spread"><h2>{order.subject || `商品 #${order.productId}`}</h2><Link to={'/products/' + order.productId}>查看商品 →</Link></div>
      <Notice>{order.status === 1 ? remaining > 0 ? `请在 ${Math.floor(remaining / 60)} 分 ${String(remaining % 60).padStart(2, '0')} 秒内完成支付。` : '支付期限已到，正在核对支付及关单结果，请勿重复付款。' : order.status === 2 ? (order.payType === 2 ? '支付宝沙箱支付已确认。' : '本地模拟支付已确认。') + '此订单未发生真实资金扣款。' : closeReason(order)}</Notice>
      ...
      <PaymentPanel key={order.id} order={order} canPay={canPay} onPaid={refresh}/>
      <button onClick={refresh} disabled={view.checking}>刷新订单状态</button>
      <Notice error>{view.error && <>{view.error} 当前保留上次结果，请刷新确认。</>}</Notice>
    </section>}
    <p className="fineprint">状态以服务端结果为准。自动查询最多 30 次，长时间停留后可手动刷新。支付仅限本地模拟或支付宝沙箱，当前不支持真实付款、退款与核销。</p>
  </>;
}
